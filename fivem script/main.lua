        ...
        if DoesEntityExist(Entity) then
            for k,v in pairs(LoadedProps) do
                if v.Prop == Entity then
                    ...
                    if Dist <= 0.1 then
                        local BrokeCamera = lib.callback.await('camerasecurity:Server:BrokeCamera', false, v.Id) 
...
-- Events
RegisterNetEvent('camerasecurity:Client:LoadPropCamera', function(Prop, Coords, Rot, TheId, ShowProp) 
    if isLogin() then
        if ShowProp then
            RequestModel(Prop)
...
RegisterNetEvent('camerasecurity:Client:RemovePropCamera', function(TheId, RemoveProp) 
    if isLogin() then
        if RemoveProp then
            for k,v in pairs(LoadedProps) do         
...
RegisterNetEvent('camerasecurity:Client:CreateNewCamera',function(Type, Job, Item)
...
RegisterNetEvent('camerasecurity:Client:ConnectCamBySignal',function()
    TabletAnimation()
    ...
    if Input then
        ...
        lib.callback('camerasecurity:Server:GetStaticCams', false, function(Result)
            if Result then
                local FindCam = false
                local DataConnect = {}
...
RegisterNetEvent('camerasecurity:Client:GetSignalPaper',function(signalcode)
...
RegisterNetEvent('camerasecurity:Client:CrashCamera',function(id)
    if InCam then
...
RegisterNetEvent('camerasecurity:Client:OpenStaticCams',function()
...
RegisterNetEvent('camerasecurity:Client:DisableInSwitchinCamActions',function()
...
RegisterNetEvent('camerasecurity:Client:LiserFixCam',function(Prop, CamCoord, Time)
    local WaitTime = true
...
RegisterNetEvent('camerasecurity:Client:OpenPersonalCamera',function(result, myIdentifier)
    if InAnim then notify('Already a tablet open', 'error') return end

    if not result or not myIdentifier then
        result, myIdentifier = lib.callback.await('camerasecurity:Server:GetStaticCams', false)
    end
                            ...
                            onSelect = function()
                                ...
                                menu3[#menu3 +1] = {
                                    ...
                                    onSelect = function()
                                        ...
                                        lib.callback('camerasecurity:Server:RegenerateSignal', false, function(data, ident)
                                            TriggerEvent('camerasecurity:Client:OpenPersonalCamera', data, ident)
                                            InAnim = false
                                            Shown = false
                                            return
                                        end, v.id, sign)
...
function GenerateRandomIPv4()
    ...
    local Result = lib.callback.await('camerasecurity:Server:GetStaticCams', false)
    if Result then
        for k, v in pairs(Result) do
            local Settings = json.decode(v.setting) 
            if Settings.IP == GeneratedIP then
                TriggerServerEvent('camerasecurity:Server:ErrorSendAlert', 'Generate IP Already Available, Try Creating New One')
        ...
        for k, v in pairs(Result) do
            ...
            if Settings.Type == 'Signal' or Settings.Type == 'Personal' then
                if Settings.Signal == signal then
                    TriggerServerEvent('camerasecurity:Server:ErrorSendAlert', 'Generate Signale Already Available, Try Creating New One')
            ...
            if Action.Type == 'Job' or Action.Type == 'Personal' then
                ...
                if Clicked and not InSwitchingCam then
                    ...
                    if #LoadingCams > 1 then
                        TriggerEvent('camerasecurity:Client:DisableInSwitchinCamActions')
            ...
            if Input then
                ...
                if Confirm == 'confirm' then 
                    ...
                    if CurrentType == 'Job' then
                        ...
                        if RecheckItem then
                            TriggerServerEvent('camerasecurity:Server:SaveNewCam', Input[1], Setting, json.encode(coords), json.encode(cameraRotation), CurrentItem)
                        else
                    ...
                    elseif CurrentType == 'Signal' then
                        ...
                        local RecheckItem = hasItem(CurrentItem)
                        if RecheckItem then
                            TriggerServerEvent('camerasecurity:Server:SaveNewCam', Input[1], Setting, json.encode(coords), json.encode(cameraRotation), CurrentItem, Setting.Signal)
                        else
                            notify('Need item to add camera', 'error', 5000)
                        end        
                        CurrentType = nil
...
function hasItem(item)
    return lib.callback.await('camerasecurity:Server:HasItem', false, item)
end
                            ...
                            local menu2 = {
                                {
                                    ...
                                    onSelect = function()
                                        ...
                                        if input then
                                            ...
                                            TriggerServerEvent('camerasecurity:Server:BuyItem', 'cash', t.price, t.item, amount)
                                        else
                                ...
                                },
                                {
                                    ...
                                    onSelect = function()
                                        ...
                                        if input then
                                            local amount = tonumber(input[1])
                                            TriggerServerEvent('camerasecurity:Server:BuyItem', 'bank', t.price, t.item, amount)
                                        else
                                            lib.showContext('camera_shop_cash_bank')
                                        end
                                    end
...
function repairCamera(id, coords, setting)
    ...
    if Dist <= setting.DistanceRemove + 1 then
        if hasItem(Config.NeedItemFixCam) then
            ...
            TriggerEvent('camerasecurity:Client:LiserFixCam', TabletObj, CamCoords, timerrr)
            ...
            InAnim = false                               
            TriggerServerEvent('camerasecurity:Server:FixCameraByID', id)  
        else
            InAnim = false 
...
function removeCamera(v, sett)
    ...
    if Dist <= sett.DistanceRemove + 1 then
        ...
        if Confirm == 'confirm' then
            InAnim = false
            TriggerServerEvent('camerasecurity:Server:RemoveStaticCam', v.id)      
        elseif Confirm == 'cancel' then
            lib.showContext('Option_Camera_Menu') 
        end  
    else
                ...
                if hasJob(k) then               
                    ...
                    if Dist <= 2 then
                        ...
                        if IsControlPressed(0, 38) then -- E
                            TriggerEvent('camerasecurity:Client:OpenStaticCams') 
                            Wait(500)
                        end
                    end
                end
